...
export function POSSection(): React.JSX.Element {
  ...
  return (
    <section id="pos" aria-label="App Caisse (POS)">
      ...
      >
        <motion.div
          initial={{ opacity: 0, y: prefersReducedMotion ? 0 : 16, rotateX: prefersReducedMotion ? 0 : -6 }}
          ...
          aria-hidden
        >
          <div className="relative mx-auto max-w-3xl overflow-hidden rounded-xl border border-white/10 bg-black/40 shadow-elev-2">
            <Image src="/window.svg" alt="" width={1200} height={700} className="opacity-80" />
            {!prefersReducedMotion ? <div className="pointer-events-none absolute inset-0 animate-pulseGlow" /> : null}
          </div>
        </motion.div>
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
          <motion.div
            className="border-border bg-card/60 rounded-xl border p-6 shadow-elev-1 backdrop-blur"
            initial="offscreen"
